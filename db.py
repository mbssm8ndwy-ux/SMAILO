...
class Database:
    ...
    def list_review_months(self) -> List[Dict]:
        with self._connect() as conn:
            rows = conn.execute("""
                SELECT strftime('%Y-%m', COALESCE(o.review_at, o.created_at)) AS month,
                       COUNT(*) AS total
                FROM orders o
                WHERE o.status = 'completed'
                  AND TRIM(COALESCE(o.review_text, '')) != ''
    ...
    def count_reviews_in_month(self, month_key: str) -> int:
        with self._connect() as conn:
            row = conn.execute("""
                SELECT COUNT(*) AS c FROM orders o
                WHERE o.status = 'completed'
                  AND TRIM(COALESCE(o.review_text, '')) != ''
                  AND strftime('%Y-%m', COALESCE(o.review_at, o.created_at)) = ?
            """, (month_key,)).fetchone()
            return int(row["c"]) if row else 0

    def list_reviews_by_month(self, month_key: str, page: int = 1, per_page: int = 5) -> List[Dict]:
        ...
        with self._connect() as conn:
            rows = conn.execute("""
                ...
                ORDER BY COALESCE(o.review_at, o.created_at) DESC, o.id DESC
                LIMIT ? OFFSET ?
            """, (month_key, limit, offset)).fetchall()
            return self._rows_to_dicts(rows)
